...
void make_csv(float time, int N, int gridsize, int thread_per_block){
  FILE* fp;
  char root_filename[] = "measure/global";
...
__global__ void computeNextQueue(int *adjacencyMatrix, int n_vertices, int *distance, int queueSize, int *currentQueue, int *nextQueueSize, int *nextQueue, int level) {
...
int main(int argc,char ** argv){
  ...
  */
  ...
  while (currentQueueSize > 0) {
		...
		computeNextQueue<<<blocks, th_p_block>>> (d_adjacencyMatrix, graph->max_neighbours, d_distance, currentQueueSize, d_currentQueue, d_nextQueueSize, d_nextQueue, level);
